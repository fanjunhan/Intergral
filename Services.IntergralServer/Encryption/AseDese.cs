...
namespace Integral.WebApi.Encryption
{
    ...
    public class AseDese
    {
        ...
        public static string DesEncrypt(string message, string key)
        {
            ...
            {
                result = StringToHex(DES(key, message, true, 0, "")).Trim();
            }
            return result;
        }
        #endregion
        ...
        public static string DesDecrypt(string message, string key)
        {
            ...
            if (!string.IsNullOrEmpty(message))
            {
                try
                {
                    result = DES(key, HexToString(message), false, 0, "");
                }
                catch
                {
                }                

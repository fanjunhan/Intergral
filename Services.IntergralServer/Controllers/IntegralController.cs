...
using YLH.Infrastructure.ExternalSign;
...
namespace Integral.WebApi.Controllers
{
  ...
  //      private const string daySql = "SELECT Score,AreaId,Day  from integral_day where  Day>=@Date and AreaId=@AreaId and ClassId is null order by Day,AreaId,ClassId";
        private const string classSql = "SELECT Score,AreaId,Day,ClassId  from integral_day where  Day>=@Date and AreaId=@AreaId and ClassId=@ClassId order by AreaId,Day,ClassId";
        private const string monthSql = "SELECT Score,AreaId,Month,ClassId  from integral_month where  Month>=@Date and AreaId=@AreaId and ClassId=@ClassId order by AreaId,Month,ClassId";
        ...
        public IntegralController(ILoggerFactory logFactory, IDapperFactory dapperFactory, IIntegralConfigStore configStore)
        {
            this.configStore = configStore;
            _logger = logFactory.CreateLogger<IntegralController>();
            if (_dapperClient == null)
                _dapperClient = dapperFactory.CreateClient("Default");
        }
        ...
        {            
            ...
            var result = await _dapperClient.QueryAsync<IntegralRecord>(intSql, new { UserId = userId, ClassId = classId,Limit = pageSize, Offset = pageIndex * pageSize });
        ...
        {
            var result = await _dapperClient.QueryFirstAsync<ValueTuple<int, int, string>>(mySql, new { AreaCode = areaCode, UserId = userId });
            return Ok(result);
        }


        [HttpGet("day/{date}")]
        public async Task<IActionResult> GetMydayclass([FromServices] DaprClient daprClient, string date, string classId, string areaId = "")
        {
            string daySql;
            if (date.Length == 10)
            {
            ...
            var result = await _dapperClient.QueryAsync<ValueTuple<int, string, string, string>>(areaId.Length == 0 ? daySql.Replace("AreaId=", "AreaId!=") : daySql, new { Date = date, ClassId= classId, AreaId = areaId });
        ...
        {
            var result = await _dapperClient.QueryAsync<ValueTuple<int, int, string>>(areaSql,"");
            return Ok(result);
        }
        ...
        {
            var result = await _dapperClient.QueryAsync<ValueTuple<int, int, int, int>>(topSql, "");
            return Ok(result);
        }
        ...
        {
            var result = await _dapperClient.QueryAsync<ValueTuple<string, int>>(areaId.Length >10 ? interSql.Replace("GROUP", "where AreaId=@AreaId GROUP") : interSql, new { Count = count, AreaId = areaId });
            return Ok(result);
        }
        ...
        public async Task<CommonPagedResponseDto<IList<TopUser>>> GetTop(string areaCode, string areaId = "", string unionId="", int pageSize = 10, int pageIndex = 1)
        {
            ...
            if (pageIndex < 1) pageIndex = 1;
            var result = await _dapperClient.QueryAsync<TopUser>(mysql,
                new { AreaCode = areaCode, AreaId = areaId, UnionId = unionId, Limit = pageSize, Offset = (pageIndex-1) * pageSize });
            
            var count = await _dapperClient.QueryFirstAsync<int>(countsql, new { areaCode, areaId, unionId });
        ...
        public async Task<IActionResult> Spend([FromBody] AppParameterDto dto)
        {
            ...
            int result = await _dapperClient.ExecuteAsync(updateSql, record);
            if (result == 0)
            {
                _logger.LogInformation(webApiInput.UserId + " 积分兑换不足");
                return Ok(false);
            }
            await _dapperClient.ExecuteAsync(spendSql, record);
            _logger.LogInformation(webApiInput.UserId + " 积分兑换花费分数：" + record.Score);
            return Ok(true);
        }
        ...
        public async Task<IActionResult> SetUserArea([FromBody] AppParameterDto dto)
        {
            ...
            int result = await _dapperClient.ExecuteAsync(updateAreaSql, areaInput);
            return Ok(result>0);
        }
        /*           /// <summary>
                  /// 获取区域积分配置信息
                  ...
                  /// <param name="areaCode">区域编码</param>
                  /// <returns></returns>
                  [HttpGet("init/{areaCode}")]
             public async Task<IActionResult> InitScore([FromServices] DaprClient daprClient, string areaCode = "51")
              {
                  var configList = await _dapperClient.QueryAsync<IntegralConfig>(ClaptrapCodes.InitSql, new { AreaCode = areaCode + "%" });
                  foreach (var ilist in configList)
                  {
                      configStore.Save(ilist);
                      var state = await daprClient.GetStateEntryAsync<IntegralConfig>(ClaptrapCodes.StoreName, ilist.ClassId);
                      state.Value = ilist;
                      await state.SaveAsync();
                  }
                  _logger.LogInformation($"初始化市州{areaCode}积分配置{configList.Count}条成功！");
        ...
        public async Task<IActionResult> SaveConfig([FromBody] AppParameterDto dto)
        {
            ...
            try
            {
                result = await _dapperClient.ExecuteAsync(InsertConfigSql, config);
                if (result > 0)
                {
